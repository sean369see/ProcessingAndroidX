...
public class PGLU {
    ...
    private static String[] glErrorStrings = new String[]{"invalid enumerant", "invalid value", "invalid operation", "stack overflow", "stack underflow", "out of memory", "invalid framebuffer operation"};
    private static String[] gluErrorStrings = new String[]{"invalid enumerant", "invalid value", "out of memory", "", "invalid operation"};
    private static String[] gluTessErrors = new String[]{" ", "gluTessBeginPolygon() must precede a gluTessEndPolygon", "gluTessBeginContour() must precede a gluTessEndContour()", "gluTessEndPolygon() must follow a gluTessBeginPolygon()", "gluTessEndContour() must follow a gluTessBeginContour()", "a coordinate is too large", "need combine callback"};
    ...
    public static String gluErrorString(int errorCode) {
        if (errorCode == 0) {
            return "no error";
        } else if (errorCode >= 1280 && errorCode <= 1286) {
            return glErrorStrings[errorCode - 1280];
        } else if (errorCode == 32817) {
            return "table too large";
        } else if (errorCode >= 100900 && errorCode <= 100904) {
            return gluErrorStrings[errorCode - 100900];
        } else {
            return errorCode >= 100151 && errorCode <= 100158 ? gluTessErrors[errorCode - 100150] : "error (" + errorCode + ")";
        }
    }
}
