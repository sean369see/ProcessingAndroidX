...
public class CompatUtils {
    ...
    */
    public static void getDisplayParams(Display display, DisplayMetrics metrics, Point size) {
        if (Build.VERSION_CODES.JELLY_BEAN_MR1 <= VERSION.SDK_INT) {
            display.getRealMetrics(metrics);
            display.getRealSize(size);
        }

        if (Build.VERSION_CODES.ICE_CREAM_SANDWICH <= VERSION.SDK_INT) {
    ...
    public static int getUniqueViewId() {
        // Build.VERSION_CODES.JELLY_BEAN_MR1 API level 17 
        if (Build.VERSION_CODES.JELLY_BEAN_MR1 <= VERSION.SDK_INT) {
